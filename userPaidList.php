...
if (empty($_POST['ramo'])) {
  ...
  $sql1 = "SELECT * FROM users, apartments , dues , payments WHERE apartments.aUserID = users.userID AND '$loginUserID' = users.userID
  AND users.userID = payments.userNo AND payments.duesID = dues.duesID
  AND apartments.apartmentIsFull=1 AND payments.isPaid = 1 ORDER BY payments.whenPaid";
